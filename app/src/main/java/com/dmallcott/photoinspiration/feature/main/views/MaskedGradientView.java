...
// IDEA: make maskedImage extend MaskedGradient so the behaviour is shared
public class MaskedGradientView extends View {
  ...
  protected void onDraw(Canvas canvas) {
    ...
    if (!TextUtils.isEmpty(header)) {
      StaticLayout staticLayout = new StaticLayout(header, textPaint, canvas.getWidth(),
          Alignment.ALIGN_CENTER, 1.0f, 0, false);
      canvas.translate((canvas.getWidth() / 2) - (staticLayout.getWidth() / 2),
          (canvas.getHeight() / 2) - ((staticLayout.getHeight() / 2)));
      staticLayout.draw(canvas);
      StaticLayout staticLayout2 = new StaticLayout(body, bodyPaint, canvas.getWidth(),
          Alignment.ALIGN_CENTER, 1.0f, 0, false);
